...
这是一个静态、local-first 的 CPQ-35 双人作答界面。核心计分采用 Crenshaw 等（2017）的 revised scoring；ECR-R、完整英文 DCI-37、情感观察和纵向分析均保持为独立模块。
...
- 完整官方英文 DCI-37 双人独立作答，固定校验分量表归属与8个反向题，报告第1–35题总分、9个分量表及2个评价题。
...
DCI 已在 `dci-config.js` 接入 Bodenmann（2008）的官方英文 DCI-37。原作者公开题本说明：学术、临床和研究用途可免费使用，复用时应引用原始 DCI，题本末尾并载明无需另行向 Guy Bodenmann 申请使用或复制许可。网站保留该版权与引用说明；这不等于授权任意商业再许可，也不覆盖未经验证的中文翻译。正式中文研究仍应取得相应版本并完成或引用测量等值性证据。
